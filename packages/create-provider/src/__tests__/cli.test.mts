...
beforeEach(async () => {
	tmpRoot = await mkdtemp(join(tmpdir(), "create-auth-provider-cli-"));
...
describe("CLI main()", () => {
	it("prints help and exits 0 for --help", async () => {
		...
		expect(code).toBe(0);
		expect(stdout.text()).toMatch(/Usage: create-auth-provider/);
	...
	it("prints version and exits 0 for --version", async () => {
		...
		expect(code).toBe(0);
		expect(stdout.text()).toMatch(/@provin-line\/create-auth-provider \d+\.\d+\.\d+/);
	});

	it("prints version for -V", async () => {
		const code = await main(["-V"], io);
		expect(code).toBe(0);
		expect(stdout.text()).toMatch(/@provin-line\/create-auth-provider/);
	});
	...
	it("returns 2 when --registry-base-url is not a URL", async () => {
		...
		expect(code).toBe(2);
		expect(stderr.text()).toMatch(/--registry-base-url/);
	});

	it("returns 2 when --registry-base-url uses an unsupported protocol", async () => {
		const code = await main(
			[
				"foo",
				"--no-git-init",
		...
		expect(code).toBe(2);
		expect(stderr.text()).toMatch(/protocol/);
	});

	it("returns 2 when --registry-base-url contains HOCON-breaking chars", async () => {
		const code = await main(
			[
				"foo",
				"--no-git-init",
		...
		expect(code).toBe(2);
		expect(stderr.text()).toMatch(/HOCON\/YAML quoting/);
	});
	...
	it("uses last segment of scoped name as default output dir", async () => {
		...
		expect(code).toBe(0);
		expect(stdout.text()).toMatch(/auth-provider/);
	});
