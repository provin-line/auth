...
// Direct-API tests for generatePolicyVerifierScaffold. Round-trip semantics
...
import { generatePolicyVerifierScaffold, substitute } from "../generator.mjs";

let tmpRoot: string;

beforeEach(async () => {
	tmpRoot = await mkdtemp(join(tmpdir(), "create-policy-verifier-gen-"));
});
...
describe("generatePolicyVerifierScaffold — file emission", () => {
...
describe("generatePolicyVerifierScaffold — template substitution", () => {
	it("substitutes __NAME__ into package.json#name", async () => {
		...
		expect(pkg.name).toBe("@example/my-policy-verifier");
	});
	...
	it("emits exact-pin dep versions (no caret)", async () => {
		...
		for (const [name, version] of Object.entries(pkg.dependencies)) {
			expect(
				...
				`${name} should be exact-pinned, got "${version}"`,
			).toBe(false);
		}
	});
	...
	it("preserves .gitkeep placeholders for src/rules and src/collectors", async () => {
		const outDir = join(tmpRoot, "out");
		const result = await generatePolicyVerifierScaffold({
			name: "test-scaffold",
			outDir,
		...
		expect(result.filesWritten).toContain("src/rules/.gitkeep");
		expect(result.filesWritten).toContain("src/collectors/.gitkeep");
	});

	it("emits valid JSON even with JSON-special chars in free-form values", async () => {
		// Codex P2 #3: a description containing quotes, a backslash, and a
		// newline must not corrupt the emitted package.json. The object-based
		// emission path escapes these via JSON.stringify.
		const outDir = join(tmpRoot, "out");
...
describe("generatePolicyVerifierScaffold — input validation", () => {
...
describe("substitute() — token totality", () => {
	it("replaces known tokens with their mapped values", () => {
		...
		expect(result).toBe("Hello world, port 3001");
	});

	it("throws on any unknown token so unmapped templates can't ship silently", () => {
		// Claude Important #3: keep the substitution total. Any template
		// author who introduces a new `__FOO__` token must wire its value
		// in buildTokens, or this assertion fires at generate time.
		expect(() =>
	...
	it("leaves single underscores and lowercase markers alone", () => {
		// The token shape is `__UPPERCASE__` — markers like `_foo_` or
		// `__lowercase__` must not be treated as tokens.
