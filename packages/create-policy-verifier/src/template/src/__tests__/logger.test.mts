...
async function firstLine(write: (logger: ReturnType<typeof createAppLogger>) => void, level?: string) {
    ...
    write(createAppLogger("dplaax-policy-verifier", level, stream));
...
describe("createAppLogger", () => {
    it("defaults to info", () => {
    	...
    	expect(createAppLogger("dplaax-policy-verifier").level).toBe("info");
    });

    it("honours LOG_LEVEL when no level is passed", () => {
    	process.env.LOG_LEVEL = "debug";
    	expect(createAppLogger("dplaax-policy-verifier").level).toBe("debug");
    });

    it("prefers an explicit level (logging.level from config) over LOG_LEVEL", () => {
    	process.env.LOG_LEVEL = "debug";
    	expect(createAppLogger("dplaax-policy-verifier", "error").level).toBe("error");
    });
    ...
    it("emits NDJSON — one parseable object per line, named for the aggregator", async () => {
    	...
    	expect(entry.name).toBe("dplaax-policy-verifier");
