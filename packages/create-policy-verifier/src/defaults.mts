...
export const DEFAULT_DEP_VERSIONS = {
	...
	"@o3co/auth.policy-verifier.server": "0.12.0",
	"@o3co/auth.policy-verifier.builtins": "0.12.0",
	"@o3co/auth.policy-verifier.core": "0.12.0",
	"@o3co/ts.hocon": "0.1.5",
	// Runtime — non-o3co
 ...
 * to this value — callers MUST pass `--dplaax-module-ref` explicitly so the
 * scaffold cannot accidentally pin to a moving target. This constant
 * remains as a stable reference for tests and for tooling
 * that knows the matching tag (e.g. a release script that bakes in the
 * just-tagged version).
 */
...
export function buildDplaaxModuleDep(ref: string): string {
	return `github:provin-line/auth#${ref}&path:/packages/policy-verifier-dplaax-module`;
}
